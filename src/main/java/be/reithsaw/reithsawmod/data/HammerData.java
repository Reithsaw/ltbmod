package be.reithsaw.reithsawmod.data;


import be.reithsaw.reithsawmod.REITHSAWMOD;
import be.reithsaw.reithsawmod.hammermaterial.CustomToolMaterial;
import be.reithsaw.reithsawmod.item.ExtendedHammerItem;
import be.reithsaw.reithsawmod.mixin.ItemGroupAccessor;
...
public class HammerData implements ContentData {
    ...
    public void register(Identifier identifier) {
        ...
        if (!isExtra() || isExtra() && REITHSAWMOD.CONFIG.enableExtraMaterials) {

            // setup settings
            Item.Settings settings = new Item.Settings().group(REITHSAWMOD.ITEM_GROUP);
            if (isFireImmune()) {
                settings.fireproof();
            }
            ...
            Identifier registryID = path.contains(":") ? new Identifier(path) : REITHSAWMOD.id(path);
